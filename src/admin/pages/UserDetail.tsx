...
export default function UserDetail() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        // 프로필별 약 수 + 최근 복약기록을 사용자 범위 안에서만 조회 (collectionGroup 인덱스 불필요)
  ...
  if (error || !user) {
    return (
      <div>
        <Link to="/dashboard/users" className="text-sm text-brand">
          ← 목록
        </Link>
  ...
  return (
    <div>
      <Link to="/dashboard/users" className="text-sm text-brand hover:text-brand-dark">
        ← 목록
      </Link>
